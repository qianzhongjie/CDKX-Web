...
namespace CDKX.Web.Areas.Admin.Controllers
{
    ...
    public class SystemController : AdminBaseController
    {
        ...
        public ActionResult GetOperateLogData()
        {
            ...
            var datas = GetQueryData<OperateLog, int>(LoggingContract.OperateLogs.Include(p => p.DataLogs), out total, request).AsEnumerable().Select(m => new
            {
                m.Id,
                m.FunctionName,
                UserId = m.Operator.UserName.FromJsonString<SysUser>().Id,
                UserName = m.Operator.UserName.FromJsonString<SysUser>().UserName,
                m.Operator.Ip,
                m.CreatedTime
            }).ToList();
